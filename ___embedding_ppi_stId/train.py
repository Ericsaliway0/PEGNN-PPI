...
def train_(hyperparams=None, data_path='gat/data/emb', plot=True):
    ...
    output_file_path = "gat/data/NCBI_protein_map.csv"
    gene_names_file_path = "gat/data/gene_names.csv"
    protein_map = create_protein_map(reactome_file_path, output_file_path)
    ...
    for data in dl_train:
        ...
        # Check and fix the mismatch
        if len(cluster_labels_initial) != len(nx_graph.nodes):
            print("Mismatch found between cluster labels and graph nodes. Attempting to align them.")

            # Ensure cluster_labels_initial and embeddings align with nx_graph nodes
            ...
            for node in nx_graph.nodes:
                if node in node_to_index_initial:
                    idx = node_to_index_initial[node]
                    if idx < len(cluster_labels_initial):  # Ensure index is within bounds
                        aligned_cluster_labels.append(cluster_labels_initial[idx])
                        aligned_embeddings.append(all_embeddings_initial[idx])
                    else:
                        # Handle out-of-bounds access, e.g., assign a default label or embedding
                        aligned_cluster_labels.append(-1)
                        aligned_embeddings.append([0] * all_embeddings_initial.shape[1])
                else:
                    # If node is not in the dictionary, add a placeholder
                    aligned_cluster_labels.append(-1)
                    aligned_embeddings.append([0] * all_embeddings_initial.shape[1])

            # Update variables
            cluster_labels_initial = aligned_cluster_labels
            all_embeddings_initial = np.array(aligned_embeddings)

        # Verify updated lengths
        assert len(cluster_labels_initial) == len(nx_graph.nodes), "Cluster labels and number of nodes still do not match"
        print("Lengths are now aligned.")

        # Proceed with the rest of your code using the aligned embeddings and labels
        node_to_index_initial = {node: idx for idx, node in enumerate(nx_graph.nodes)}
        ...
        stid_dic_initial = {}
            ...
            # Initialize the dictionary to hold the stId to embeddings mapping
        stid_dic_initial = {}

        # Populate stid_dic with node stIds mapped to embeddings
        for node in nx_graph.nodes:
            # Check if the node has the 'stId' attribute
            if 'stId' in nx_graph.nodes[node]:
                stid = nx_graph.nodes[node]['stId']
                # Ensure that the node has an index in the all_embeddings_initial list
                if node in node_to_index_initial:
                    # Map the stId to the corresponding embedding
                    stid_dic_initial[stid] = all_embeddings_initial[node_to_index_initial[node]]
            else:
                print(f"Node {node} does not have an 'stId' attribute.")
        ...
        for node, cluster in zip(nx_graph.nodes, cluster_labels_initial):
            if cluster not in first_node_stId_in_cluster_initial:
                ...
                first_node_embedding_in_cluster_initial[cluster] = all_embeddings_initial[node_to_index_initial[node]]
    ...
    for data in dl_train:
        ...
        for node in nx_graph.nodes:
            ...
            if 'stId' in nx_graph.nodes[node]:
                stid = nx_graph.nodes[node]['stId']
                # Ensure that the node has an index in the all_embeddingslist
                if node in node_to_index:
                    # Map the stId to the corresponding embedding
                    stid_dic[stid] = all_embeddings[node_to_index[node]]
            else:
                print(f"Node {node} does not have an 'stId' attribute.")
    ...
    # Save to CSV
    ##csv_save_path = 'gat/data/protein_embeddings_final_sage.csv'
    csv_save_path = 'gat/data/protein_embeddings_final_gat.csv'
    stid_df_final.to_csv(csv_save_path, index_label='stId')'''
...
def train(hyperparams=None, data_path='gat/data/emb', plot=True):
    ...
    for data in dl_train:
        ...
        # Convert stid_dic_initial to a DataFrame
        stid_df = pd.DataFrame.from_dict(stid_dic_initial, orient='index')

        # Save to CSV
        ##csv_save_path = 'gat/data/protein_embeddings_initial_sage.csv'
        csv_save_path = 'gat/data/protein_embeddings_initial_gat.csv'
        stid_df.to_csv(csv_save_path, index_label='stId')
    ...
    save_to_json(graph_train, stid_dic_initial, stid_mapping, protein_map, gene_id_to_name_mapping, gene_id_to_symbol_mapping, results_path)
    '''
    
    ##save_to_json(graph_train, stid_dic, stid_mapping, protein_map, gene_id_to_name_mapping, gene_id_to_symbol_mapping, results_path)

    ##  save_to_neo4j(graph_train, stid_dic, stid_mapping, protein_map, gene_id_to_name_mapping, gene_id_to_symbol_mapping, neo4j_uri, neo4j_user, neo4j_password)
       
    '''protein_embeddings_initial = pd.DataFrame.from_dict(all_embeddings_initial)
    protein_embeddings_initial.to_csv('gat/data/protein_embeddings_initial.csv', index_label='stId')

    ##protein_embeddings_final = pd.DataFrame.from_dict(all_embeddings, orient='index')
    protein_embeddings_final = pd.DataFrame.from_dict(all_embeddings)
    protein_embeddings_final.to_csv('gat/data/protein_embeddings_final.csv', index_label='stId')'''
...
def plot_cosine_similarity_matrix_for_clusters_with_values(embeddings, stids, save_path):
    ...
    ax.text(x=0.5, y=-0.03, s="Pathway-protein similarities", fontsize=12, ha='center', va='top', transform=ax.transAxes)

    plt.savefig(save_path)
    ##plt.show()
    plt.close()
    
def create_protein_map(reactome_file, output_file):
    """
    ...
    """
    protein_map = {}  # Dictionary to store gene IDs for each protein STID

    # Read the NCBI2Reactome.csv file and populate the protein_map
    with open(reactome_file, 'r') as file:
        reader = csv.reader(file, delimiter='\t')
        for row in reader:
            gene_id = row[0]
            protein_stid = row[1]
            protein_map.setdefault(protein_stid, []).append(gene_id)

    # Write the protein_map to the output CSV file
    with open(output_file, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Pathway STID", "Gene IDs"])  # Write header
        for protein_stid, gene_ids in protein_map.items():
            writer.writerow([protein_stid, ",".join(gene_ids)])
    
    return protein_map
        
def save_to_neo4j(graph, stid_dic, stid_mapping, protein_map, gene_id_to_name_mapping, gene_id_to_symbol_mapping, uri, user, password):
    from neo4j import GraphDatabase
    ...
    try:
        ...
        for node_id in stid_dic:
            ...
            ##genes = get_genes_by_protein_stid(node_id, reactome_file, gene_names_file)
            genes = protein_map.get(node_id, [])
